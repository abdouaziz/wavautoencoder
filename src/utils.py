...
def compute_mask_indices(
    ...
    spec_aug_mask = torch.zeros((batch_size, sequence_length), dtype=torch.bool, device=input_feature.device)

    # uniform distribution to sample from, make sure that offset samples are < sequence_length
    uniform_dist = torch.ones((batch_size, sequence_length - (mask_length - 1)), device=input_feature.device)
    ...
    offsets = torch.arange(mask_length, device=input_feature.device).unsqueeze(0).expand((num_masked_tokens, mask_length))
